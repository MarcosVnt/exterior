        ...
        <div class="card-body">
          ...
          <div class="table-responsive">
            <div style="padding: 15px; display: block; min-height: 65px">
              ...
              >
                <i class="icon-plus"></i>&nbsp;Nuevo Producto Tarifa
              </button>
            </div>
          </div>
          ...
          <div class="container">
            <div
              class="row li2"
              v-for="(articulo, idx) in articulos"
              :key="idx"
              v-on:click="toggleDetails(articulo, idx)"
              v-bind:class="[articulo.mostrar ? 'less' : 'more', 'book']"
            >
           <!--   <button
                  v-if="articulo.mostrar"
                  type="button"
                  class="close"
                  data-dismiss="modal"
                  @click="toggleDetails(articulo,idx)"
                  aria-label="Close"
                >
                  <span aria-hidden="true">×</span>
                </button>  --> 

              <div class="col-4 text-titulo">{{ articulo.referencia }}</div>
              <div class="col-6 text-titulo">{{ articulo.descripcion }}</div>


               <div class="table-responsive" v-show="articulo.mostrar">
                <!--  <p>{{ articulo.descripcion }}</p>
                <p v-if="articulo.descripcion" class="byline">By: {{articulo.descripcion}}</p>
              -->
                <table
                  id="table-fixed-last-column16"
                  class="table table-responsive table-bordered table-striped2 table-sm"
                >
                  <thead>
                    <tr style>
         ...
         <!--  <ul class="ul2">
            ...
            >
              ...
              <h6>{{ articulo.referencia }}</h6>

              <div v-show="articulo.mostrar">
...
export default {
  ...
  data() {
    return {
      ...
      showDetail: true,
      articuloMarcado: [],
      articuloMarcadoEncontrado: false,
  ...
  methods: {
    articuloMarcadoExiste(articulo) {
      console.log("articuloMarcadoExiste", articulo);
      if (this.articuloMarcado.find(articulo)) {
        return true;
      } else {
        return false;
      }
    ...
    toggleDetails: function (articulo, idx) {
      console.log(articulo, idx, this.articulos, this.articulos[idx]);

      articulo.mostrar = !articulo.mostrar;

      console.log(articulo, idx, this.articulos);
